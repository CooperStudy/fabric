...
func ComputeSHA256(data []byte) (hash []byte) {
	//logger.Info("===ComputeSHA256================")
	def := factory.GetDefault()
	//fmt.Printf("==factory.GetDefault() type:%T==",def)
	//logger.Info("====ComputeSHA256=======",def)
	//logger.Info("====data",data)
	//logger.Info("====&bccsp.SHA256Opts{}",&bccsp.SHA256Opts{})
	hash, err := def.Hash(data, &bccsp.SHA256Opts{})
	//logger.Info("===========hash",hash)//[213 110 223 124 231 48 20 241 34 225 248 10 249 2 198 177 223 223 99 180 197 126 160 39 81 101 28 155 126 75 234 202]
	//===CSP===
...
func CreateUtcTimestamp() *timestamp.Timestamp {
	logger.Info("===CreateUtcTimestamp================")
	now := time.Now().UTC()
	secs := now.Unix()
...
func GenerateHashFromSignature(path string, args []byte) []byte {
	logger.Info("===GenerateHashFromSignature================")
...
func GenerateIDfromTxSHAHash(payload []byte) string {
	logger.Info("===GenerateIDfromTxSHAHash================")
...
func FindMissingElements(all []string, some []string) (delta []string) {
	logger.Info("===FindMissingElements================")
...
func ArrayToChaincodeArgs(args []string) [][]byte {
	logger.Info("===ArrayToChaincodeArgs================")
	bargs := make([][]byte, len(args))
	for i, arg := range args {
		bargs[i] = []byte(arg)
	}
