...
func (c Config) String() string {
	...
	if c.AuthCacheEnabled {
		...
		logger.Info("==c.AuthCacheEnabled======", a)
		return a
	}
	a = fmt.Sprintf("TLS: %t, auth cache disabled", c.TLS)
	logger.Info("==a======", a) //TLS: true, auth cache disabled
	return a
}
...
func (s *service) processQuery(query *discovery.Query, request *discovery.SignedRequest, identity []byte, addr string) *discovery.QueryResult {
	logger.Info("===service===processQuery==")
	//logger.Info("========query.Channel==========", query.Channel)//mychannel
	//logger.Info("===========!s.ChannelExists(query.Channel)=============", s.ChannelExists(query.Channel)) // true
	...
	*/
	//logger.Info("===========request.Signature========",request.Signature)
	/*
	...
	*/
	//logger.Info("===========request.identity========",identity)
...
func (s *service) dispatch(q *discovery.Query) *discovery.QueryResult {
	...
	if q.Channel == "" {
		...
		logger.Info("=========s.localDispatchers=====", s.localDispatchers)
	}
	logger.Info("========q.GetType()======", q.GetType()) //1
	dispatchQuery, exists := dispatchers[q.GetType()]
	logger.Info("=========dispatchQuery==================", dispatchQuery) //0xd54d80
	logger.Info("================exists==============", exists)            //true
	if !exists {
...
func (s *service) chaincodeQuery(q *discovery.Query) *discovery.QueryResult {
	logger.Info("===service===chaincodeQuery==")
	...
	for _, interest := range c {
		...
		//logger.Info("=====================q.Channel==============",q.Channel)//mychannel
		//logger.Info("=====================common2.ChainID(q.Channel)==============",common2.ChainID(q.Channel))//[109 121 99 104 97 110 110 101 108]
...
func (s *service) configQuery(q *discovery.Query) *discovery.QueryResult {
	logger.Info("===service===configQuery==")
	logger.Info("===q.Channel=====",q.Channel)
...
func wrapPeerResponse(peersByOrg map[string]*discovery.Peers) *discovery.QueryResult {
	logger.Info("===wrapPeerResponse==")
	logger.Info("=============peersByOrg=============", peersByOrg)
	return &discovery.QueryResult{
...
func (s *service) channelMembershipResponse(q *discovery.Query) *discovery.QueryResult {
	logger.Info("==service===channelMembershipResponse==")

	//logger.Info("============common2.ChainID(q.Channel)==========", common2.ChainID(q.Channel))//[109 121 99 104 97 110 110 101 108]
	logger.Info("==========q.GetPeerQuery().Filter==", q.GetPeerQuery().Filter)
	chanPeers, err := s.PeersAuthorizedByCriteria(common2.ChainID(q.Channel), q.GetPeerQuery().Filter)
	//logger.Info("==========chanPeers=========", chanPeers)
	if err != nil {
		return wrapError(err)
	}
	membersByOrgs := make(map[string]*discovery.Peers)
	chanPeerByID := discovery2.Members(chanPeers).ByID()
	//logger.Info("=====chanPeerByID======",chanPeerByID)
	for org, ids2Peers := range s.computeMembership(q) {

		//logger.Info("==========org========",org)//Org1MSP
		//logger.Info("==========ids2Peers========",ids2Peers)
		...
		for id, peer := range ids2Peers {
			...
			//logger.Info("=========stateInfoMsg========",stateInfoMsg)
...
//	logger.Info("=======membersByOrgs===============",membersByOrgs)
/*
...
func (s *service) localMembershipResponse(q *discovery.Query) *discovery.QueryResult {
	logger.Info("==service===localMembershipResponse==")
	membersByOrgs := make(map[string]*discovery.Peers)
	for org, ids2Peers := range s.computeMembership(q) {
...
func (s *service) computeMembership(_ *discovery.Query) map[string]peerMapping {
	logger.Info("==service===computeMembership==")
	peersByOrg := make(map[string]peerMapping)

	p := s.Peers()
	logger.Info("========s.Peers()====", p)
	/*
	...
	*/
	...
	logger.Info("==========peerAliveInfo============",peerAliveInfo)
	for org, peerIdentities := range s.IdentityInfo().ByOrg() {
		//logger.Info("===========org========",org)
		//logger.Info("======peerIdentities======",peerIdentities)
		...
		for _, id := range peerIdentities {
			...
			//a := string(id.PKIId)
			//logger.Info("===========id.PKIId===========",a)
			aliveInfo, exists := peerAliveInfo[string(id.PKIId)]

			//logger.Info("==========aliveInfo=============",aliveInfo)
			//logger.Info("==========exists=============",exists)
			/*
			...
			*/
			...
			//logger.Info("=========id.PKIId==========",string(id.PKIId))
			/*
			...
			*/
			//logger.Info("==============id.Identity===========",id.Identity)
			/*
			...
			*/
			//logger.Info("==============aliveInfo.Envelope===========",aliveInfo.Envelope)
			/*
			...
			*/
			...
			logger.Info("=====peersForCurrentOrg=========",peersForCurrentOrg)
		}
	}
