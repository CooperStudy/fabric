...
func checkState(t *testing.T, stub *shim.MockStub, name string, expect string) {
	...
	if string(bytes) != expect {
		logger.Info("State value", name, "was not", expect, "as expected")
		t.FailNow()
	}
}
...
func checkQuery(t *testing.T, stub *shim.MockStub, args [][]byte, expect string) {
	...
	if res.Status != shim.OK {
		logger.Info("Query", args, "failed", string(res.Message))
		t.FailNow()
	}
	if res.Payload == nil {
		logger.Info("Query", args, "failed to get result")
		t.FailNow()
	}
	if string(res.Payload) != expect {
		logger.Info("Query result ", string(res.Payload), "was not", expect, "as expected")
		t.FailNow()
	}
}
