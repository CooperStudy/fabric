...
func newIdentity(cert *x509.Certificate, pk bccsp.Key, msp *bccspmsp) (Identity, error) {
	logger.Info("====func newIdentity(cert *x509.Certificate, pk bccsp.Key, msp *bccspmsp) (Identity, error)==")
	...
	//logger.Info("=====func newIdentity(cert *x509.Certificate, pk bccsp.Key, msp *bccspmsp)==================")
	//logger.Info("====cert.Raw==================",cert.Raw)
	...
	//logger.Info("=====id:==hex.EncodeToString(digest)==================", hex.EncodeToString(digest))
	if err != nil {
	...
	logger.Info("==IdentityIdentifier:MspId,Id======")
...
func (id *identity) Serialize() ([]byte, error) {
	logger.Info("==func (id *identity) Serialize() ([]byte, error)=")
	...
	//logger.Info("=======sId := &msp.SerializedIdentity{Mspid: id.id.Mspid, IdBytes: pemBytes}====================")
	//logger.Infof("========func(id *identity) Serialize()  Mspid:%v,IdBytes:pemBytes:%v===================",id.id.Mspid,pemBytes)
	//logger.Infof("=======pemBytes:%v===================",pemBytes)
